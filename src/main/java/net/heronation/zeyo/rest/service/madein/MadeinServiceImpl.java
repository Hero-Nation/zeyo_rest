...
public class MadeinServiceImpl implements MadeinService {
	...
	public Map<String, Object> use_list(Predicate where, Pageable page) {
		...
		// String start = (String)param.get("start");
		// if(start != null ) {
		// where_query.append(" AND m.create_dt >= STR_TO_DATE('"+start+"', '%Y-%m-%d
		// %H:%i:%s')");
		// }
		//
		//
		// String end = (String)param.get("end");
		// if(end != null ) {
		// where_query.append(" AND m.create_dt <= STR_TO_DATE('"+end+"', '%Y-%m-%d
		// %H:%i:%s')");
		// }
		...
		//
		//
		// PathBuilder<Madein> madeinPath = new PathBuilder<Madein>(Madein.class,
		// "madein");
		//
		...
		// query.select(mi.id,mi.name,mi.kindof.kvalue,mi.createDt,i.id.countDistinct())
		// .from(mi)
		// .where(where)
